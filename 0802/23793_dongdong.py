...
INF = 500000000000000  # 정수 최댓값 근데 왜 하는걸까..??
    ...
    while q:
        if y == 1:
            ...
            for next in graph[node]:
                cost = distances[node] + next[1]  # 시작->node의 거리 + node->node의 인접노드 거리
                if cost < distances[next[0]]:   # cost < 시작->node의 인접노드 거리
                    distances[next[0]] = cost   # 최단거리 갱신
